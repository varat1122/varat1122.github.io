...
const H={us:'Symbol หุ้นอ้างอิง',un:'อ้างอิงหุ้น',desc:'รายละเอียดธุรกิจ',dr:'DR Symbol',ct:'ที่ตั้งธุรกิจ',mk:'จดทะเบียนในตลาดหลักทรัพย์',ra:'อัตราแปลงสภาพ',biz:'ธุรกิจหลัก (สั้น)',sector:'Sector',theme:'Thematic Category',ai:'AI / Non AI'};
const DB_KEY='fdr-db-v13',PRICE_KEY='fdr-p',FX_KEY='fdr-fx',DR_PRICE_KEY='fdr-drp';
...
const cleanHeader=x=>String(x??'').replace(/\u00a0/g,' ').replace(/\s+/g,' ').trim();
const normalizeMarket=x=>{const s=String(x??'').trim().toUpperCase();return s==='J'?'JP':s};
const normalizeAI=x=>{const s=String(x??'').replace(/\s+/g,' ').trim().toUpperCase();if(s==='AI')return'AI';if(s==='NON AI'||s==='NON-AI'||s==='NONAI')return'Non AI';return s?String(x).trim():'Unknown'};
...
function ratio(v){
  ...
  return {raw,upd:+m[2]/+m[1]};
}
function headerIndex(headers,map){
  const ix={};
  for(const[k,v]of Object.entries(map))ix[k]=headers.findIndex(x=>x===v||(k==='ra'&&x.startsWith(v)));
  return ix;
}
function parseDataMeta(w){
  const sh=w.Sheets.Data;if(!sh)return new Map();
  const a=XLSX.utils.sheet_to_json(sh,{header:1,defval:'',raw:false,blankrows:false});
  const hd=(a[0]||[]).map(cleanHeader),ix=headerIndex(hd,H),map=new Map();
  if(ix.us<0)return map;
  for(let i=1;i<a.length;i++){
    const row=a[i]||[],g=k=>ix[k]>=0?String(row[ix[k]]??'').trim():'',us=g('us').toUpperCase();
    if(!us)continue;
    map.set(us,{underlyingName:g('un'),description:g('desc'),country:g('ct'),market:normalizeMarket(g('mk'))});
  }
  return map;
}
function parse(buf,name){
  ...
  const a=XLSX.utils.sheet_to_json(sh,{header:1,defval:'',raw:false,blankrows:false});
  const hd=(a[0]||[]).map(cleanHeader),ix=headerIndex(hd,H),meta=parseDataMeta(w);
  for(const k of['us','dr','ra'])if(ix[k]<0)throw Error('ไม่พบคอลัมน์: '+H[k]);
  const r=[],warnings=[],seen=new Set();
  for(let i=1;i<a.length;i++){
    const row=a[i]||[],g=k=>ix[k]>=0?String(row[ix[k]]??'').trim():'',dr=g('dr').toUpperCase(),us=g('us').toUpperCase();
    if(!dr&&!us)continue;
    if(!us){warnings.push('แถว '+(i+1)+': ไม่มี Symbol หุ้นอ้างอิง');continue}
    if(!dr){warnings.push('แถว '+(i+1)+': ไม่มี DR Symbol');continue}
    if(seen.has(dr)){warnings.push('แถว '+(i+1)+': DR ซ้ำ '+dr);continue}seen.add(dr);
    let q;try{q=ratio(g('ra'))}catch(e){warnings.push('แถว '+(i+1)+': '+e.message);continue}
    const m=meta.get(us)||{},market=normalizeMarket(g('mk')||m.market),country=g('ct')||m.country||'';
    if(!market){warnings.push('แถว '+(i+1)+': ไม่พบตลาดของ '+us+' (ต้องมีชีต Data หรือคอลัมน์ตลาดในชีต DR)');continue}
    const currency=MC[market]||'';
    if(!currency)warnings.push('แถว '+(i+1)+': ไม่รู้สกุลเงินสำหรับตลาด '+market+' ('+us+')');
    r.push({
      excelRow:i+1,underlyingSymbol:us,underlyingName:g('un')||m.underlyingName||us,
      description:g('desc')||m.description||'',drSymbol:dr,country,market,currency,
      ratioRaw:q.raw,underlyingPerDr:q.upd,businessShort:g('biz'),sector:g('sector'),
      theme:g('theme'),ai:normalizeAI(g('ai'))
    });
  }
...
function defaultDb(){
  ...
  const records=pack.r.map((row,i)=>{
    const u=pack.u[row[0]]||[];
    return {excelRow:i+2,underlyingSymbol:u[0]||'',underlyingName:u[1]||'',description:'',drSymbol:row[1]||'',country:u[7]||'',market:u[2]||'',currency:u[3]||'',ratioRaw:row[2]||'',underlyingPerDr:Number(row[3])||0,businessShort:u[4]||'',sector:u[5]||'',theme:u[6]||'',ai:normalizeAI(u[8])};
  });
  return {filename:'DR_040926.xlsx',sheet:'DR',recordCount:records.length,underlyingCount:pack.u.length,records,warnings:[]};
}
...
function setEnabled(v){['getfx','psearch','dsearch','themeFilter','aiFilter'].forEach(id=>{if($(id))$(id).disabled=!v})}
...
function apply(db,persist=true){
  S.db=db;S.r=(db.records||[]).map(x=>({...x,ai:normalizeAI(x.ai)}));
  const map=new Map();
  for(const x of S.r){
    const key=K(x.underlyingSymbol,x.market);
    if(!map.has(key))map.set(key,{key,symbol:x.underlyingSymbol,name:x.underlyingName,market:x.market,currency:x.currency,count:0,ai:x.ai});
    const u=map.get(key);u.count++;if(u.ai!==x.ai)u.ai='Mixed';
  }
  S.u=[...map.values()].sort((a,b)=>a.symbol.localeCompare(b.symbol));
  if(persist)localStorage.setItem(DB_KEY,JSON.stringify({...db,records:S.r}));
  const themes=[...new Set(S.r.map(x=>x.theme).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'th'));
  const aiCount=S.r.filter(x=>x.ai==='AI').length,nonAiCount=S.r.filter(x=>x.ai==='Non AI').length;
  $('dbinfo').innerHTML=`<b>${esc(db.filename||'ฐานข้อมูล DR')}</b><br>${S.r.length} DR · ${S.u.length} หุ้นแม่ · ${themes.length} Thematic · AI ${aiCount} · Non AI ${nonAiCount}`;
...
function renderSelected(){
  ...
  const u=S.selected;box.classList.add('show');$('selectedSymbol').textContent=u.symbol;$('selectedName').textContent=u.name+' · '+u.market+' · '+u.currency+' · '+u.ai;$('selectedPrice').value=S.p[u.key]??'';$('selectedFxLabel').textContent=u.currency+' → THB';$('selectedFx').value=S.fx[u.currency]?.rate??'';
...
function renderDirectory(){
  const q=$('dsearch').value.toLowerCase().trim(),theme=$('themeFilter').value,ai=$('aiFilter').value;
  const list=S.r.filter(x=>{
    const hay=[x.drSymbol,x.underlyingSymbol,x.underlyingName,x.businessShort,x.sector,x.theme,x.ai,x.country,x.market].join(' ').toLowerCase();
    return(!q||hay.includes(q))&&(!theme||x.theme===theme)&&(!ai||x.ai===ai);
  });
  $('dtbody').innerHTML=list.length?list.map(x=>`<tr><td class="sym">${esc(x.drSymbol)}</td><td><span class="sym">${esc(x.underlyingSymbol)}</span><span class="sub">${esc(x.underlyingName)}</span></td><td>${esc(x.businessShort||'—')}</td><td><span class="sector-pill">${esc(x.theme||'—')}</span></td><td>${esc(x.sector||'—')}</td><td><span class="ai-pill ${x.ai==='AI'?'ai':'non-ai'}">${esc(x.ai||'Unknown')}</span></td><td>${esc(x.country)}</td><td>${esc(x.market)}</td><td class="num">${esc(x.ratioRaw)}</td></tr>`).join(''):'<tr><td colspan="9" class="empty">ไม่พบรายการ</td></tr>';
  const filters=[theme||'ทุก Thematic',ai||'AI + Non AI'];
  $('dfoot').innerHTML=`<span>แสดง ${list.length} จาก ${S.r.length} DR</span><span>${filters.map(esc).join(' · ')}</span>`;
}
...
$('dsearch').addEventListener('input',renderDirectory);$('themeFilter').addEventListener('change',renderDirectory);$('aiFilter').addEventListener('change',renderDirectory);$('getfx').onclick=getfx;
$('file').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{apply(parse(await f.arrayBuffer(),f.name),true);$('status').textContent='อัปเดตฐานข้อมูลแล้ว'}catch(err){message([err.message,'ไฟล์ใหม่รองรับโครงสร้าง DR + Data แบบ DR_040926.xlsx'])}e.target.value=''};
